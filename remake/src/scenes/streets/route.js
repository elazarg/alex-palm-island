import { parseStreetStateParams, pickStreetRouteState, serializeStreetStateParams } from './state.js';

export function defaultStreetRoute(scene) {
  return {
    scene,
    view: 'scene',
    initial: false,
    ...
    state: parseStreetStateParams(new URLSearchParams()),
  };
}

export function createStreetRouteFns(scene) {
  function normalize(route = {}) {
    const base = defaultStreetRoute(scene);
    const normalized = {
      ...base,
      ...route,
      scene,
      initial: route.initial === true,
      entry: typeof route.entry === 'string' && route.entry ? route.entry : null,
      state: route.state ? pickStreetRouteState(route.state) : base.state,
    };
  ...
  return {
    defaultRoute: () => defaultStreetRoute(scene),
    normalize,
    parse: (segments = [], params = new URLSearchParams()) => {
      const route = defaultStreetRoute(scene);
      route.initial = params.get('initial') === '1';
      route.entry = params.get('entry') || null;
      route.state = parseStreetStateParams(params);
    ...
    format: (route = {}) => {
      ...
      const params = serializeStreetStateParams(normalized.state);
      if (normalized.initial) params.set('initial', '1');
      if (normalized.initial && normalized.entry) params.set('entry', normalized.entry);
      return { segments, params };
    },
    resolveInitialScreen: (route = {}) => {
      const normalized = normalize(route);
    ...
    buildFromRuntime: ({ modal = null, state = {}, initial = false } = {}) => {
      const route = {
        scene,
        view: 'scene',
        initial: initial === true,
        state: pickStreetRouteState(state),
      };
